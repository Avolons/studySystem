...
var App = new Vue({
    ...
    methods: {
        ...
        //******是否对题目进行了标记
        signtoggle: function signtoggle(item) {
            //同时要对右侧视图进行更新
            if (item.issign) {
                item.issign = false;
        ...
        stateSel: function stateSel(item, index) {
            //状态栏的操作，被选中状态
        ...
        //******点击上一题，下一题
        getNewQest: function getNewQest(direction) {
            //direction为true，为下一题
            if (direction) {
        ...
        //******点击显示对应题目
        rightcheck: function rightcheck(index, type) {
            //******首先对题目的显示进行操作
        ...
        answerchange: function answerchange(item, ite) {
            switch (ite.Type) {
                //******题型为多选题时
                case 3:
        ...
        //*******答案的预先格式化
        answerformatting: function answerformatting(arrys) {
        ...
        addLeaveTimes: function addLeaveTimes() {
            //AddDepartureTimes
            //******发送时间
            $.post("http://192.168.1.102/home/ReduceExaminationTime", {
                userExamId: this.qstData.Id, //考试id
                second: this.numbertime }, function () {});
        },
        //******以下都是对答案的处理函数
        //******发送答案，绑定提交试卷
        sendanswer: function sendanswer(url) {
            ...
            $.post(url, {
                body: arrobj,
                userExamId: this.qstData.Id, //考试id
                currentQuestionIndex: this.showIndex }, function () {});
            //ReduceExaminationTime 当前剩余考试时间 userExamid考试id，second,时间
            //******当前剩余时间
            $.post("http://192.168.1.102/home/ReduceExaminationTime", {
                userExamId: this.qstData.Id, //考试id
        ...
        //******取回答案的预先格式化
        getOldanswer: function getOldanswer(arrobj) {
            for (var i = 0; i < arrobj.length; i++) {
                //多选题的数组对象要改成字符串对象
                if (arrobj[i].Type === 3) {
        ...
        //******初次进入考试系统的时的初始化工作
        firstInitialization: function firstInitialization(data, index) {
            //答案对象初始化
        ...
        this.$http.get("http://192.168.1.102/home/GetExamInfo").then(function (data) {
            ...
            if (data.body.ShowQuestoinType !== 0) {
                ...
                if (data.body.IsFirstAnswer) {
                    _this.firstInitialization(data, index);
                    for (var i = 0; i < _this.qstData.QuestionsItems.length; i++) {
                        //当前显示题目的index
                        if (i !== _this.showIndex) {
                            _this.qstData.QuestionsItems[i].isshow = true;
                        }
                    }
                } else {
                    _this.$http.get("http://192.168.1.102/home/GetUserExamAnswer").then(function (result) {
                        _this.showIndex = data.body.CurrentQuestionIndex;
                        _this.qstData = data.body;
                        _this.answerList = _this.getOldanswer(result.body);
                        for (var i = 0; i < _this.qstData.QuestionsItems.length; i++) {
                            //当前显示题目的index
                            if (i !== _this.showIndex) {
                                _this.qstData.QuestionsItems[i].isshow = true;
                        ...
                        //数目统计初始化
                        _this.rightType[0].val = data.body.QuestionsItems.length;
                        _this.rightType[2].val = data.body.QuestionsItems.length;
                        _this.numbertime = data.body.ExamLength * 60;
                        _this.timeover();
                        // this.saveanswer();
                        layer.close(index);
                    }, function (error) {});
                ...
                if (data.body.IsFirstAnswer) {
                    ...
                    _this.$http.get("http://192.168.1.102/home/GetUserExamAnswer").then(function (result) {
                        ...
                        _this.answerList = _this.getOldanswer(result.body);
                        //*******右侧统计数据初始化
                        for (var _i = 0; _i < _this.answerList.length; _i++) {
                            _this.answerchange(_this.answerList[_i], _this.qstData.QuestionsItems[_i]);
                        }
                        //数目统计初始化
                        _this.rightType[0].val = data.body.QuestionsItems.length;
                        _this.rightType[2].val = data.body.QuestionsItems.length;
